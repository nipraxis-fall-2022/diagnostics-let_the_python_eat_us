...
def mahal(img):
    ...
    mean_vox = np.mean(data_reshaped, axis=-1) #  thank you for renaming Alex ! I keep using volume for both 3d and 4d for some reason :/
    mean_mean = np.mean(data_reshaped) # changed it - good fix !
    ...
    for vol_no in range(data.shape[-1]): #for each volume in the time series
        this_vx = data[:, vol_no] # this returns a set of voxel values
        # covariance of volume
        cov = np.cov(this_vx.T)
        ...
        # next, we take away the mean of voxel data from current volume
        this_vx_minus_mean = this_vx - np.mean(data, axis=1) # changing description here too to avoid confusion :)
        ...
        # so going for .dot
        mahal_distances = (np.sqrt(this_vx_minus_mean.T.dot(pow(cov, -1)).dot(this_vx_minus_mean)))
